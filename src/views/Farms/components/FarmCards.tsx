...
const FarmCards: React.FC = () => {
  ...
  const giveIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'GIVE',
  )

  console.log(stakedValue);

  const givePrice =
    giveIndex >= 0 && stakedValue[giveIndex]
      ? stakedValue[giveIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const GIVE_PER_BLOCK = new BigNumber(100)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [give, lpTokenAddress, account, setHarvestable])
