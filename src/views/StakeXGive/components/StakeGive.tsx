...
const StakeGive: React.FC<StakeProps> = ({}) => {
  const tokenName = "GIVE"
  const [requestedApproval, setRequestedApproval] = useState(false)

  const allowance = useAllowanceStaking()
  const {onApprove} = useApproveStaking()

  const tokenBalance = useTokenBalance(contractAddresses.give[CHAIN_ID])
      ...
      <CardContent>
        <StyledCardContentInner>
          <StyledCardHeader>
            ...
            <Label text={`GIVE Tokens Available`}/>
          </StyledCardHeader>
          <StyledCardActions>
            {!allowance.toNumber() ? (
              <Button
